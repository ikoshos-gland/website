...
const PhotoCard: React.FC<PhotoWork> = ({ title, category, location, gear, image, isFeatured }) => (
  <div className="group cursor-pointer bg-[#0E0F11] p-6 hover:bg-[#15171B] transition-colors">
    <div className="relative aspect-square bg-[#15171B] rounded-2xl overflow-hidden mb-5 border border-[#23252B]">
      <img src={image} alt={title} className="opacity-80 group-hover:opacity-100 group-hover:scale-105 transition-all duration-700 w-full h-full object-cover" />
      {isFeatured && (
    ...
    <div className="flex justify-between items-start mb-2">
      <div>
        <h3 className="text-lg font-heading font-medium text-[#F5F5F5] tracking-tight">{title}</h3>
        <p className="uppercase text-[10px] text-[#A1A1A6] tracking-widest">{category}</p>
      </div>
    </div>
    <div className="flex justify-between items-center text-xs text-[#52525B] mt-4 pt-4 border-t border-[#23252B]">
      <div className="flex items-center gap-2 group-hover:text-[#D6FF4F] transition-colors">
        <MapPin size={12} />
...
const CaseStudies: React.FC = () => {
  const fallbackPhotos: PhotoWork[] = [
    ...
    { id: "4", title: "Mısırcı", category: "Neon / Night", location: "İzmir", gear: "Nikon D3100", image: "https://i.ibb.co/jPtScSHm/DSC-0232-Geli-tirilmi-SR.jpg" },
    { id: "5", title: "Bottik", category: "Architecture", location: "Beykoz, İstanbul", gear: "Nikon D3100", image: "https://i.ibb.co/nqv57Lv6/bottikkedy.jpg", isFeatured: true },
    { id: "6", title: "Ephemeral", category: "Abstract", location: "Şirince, İzmir", gear: "Nikon D3100", image: "https://i.ibb.co/7Nkv5kv6/g-ne-28.jpg" },
  ];
  ...
  return (
    <div className="border-t border-[#23252B] bg-[#0E0F11]">
      <div className="max-w-[1550px] mx-auto grid grid-cols-1 md:grid-cols-12">
        ...
        {/* Sidebar Info */}
        <div className="md:col-span-3 p-6 md:p-8 md:border-r border-[#23252B] flex flex-col justify-between h-auto md:min-h-[600px] relative">
          <div className="md:sticky md:top-48 z-10">
            ...
            {/* Crazy Vertical Text - Replaces Description */}
            <div className="hidden md:flex mt-8 select-none justify-start">
              <span
                className="text-5xl lg:text-6xl font-bold text-[#2A2C33] hover:text-[#D6FF4F] transition-colors duration-500 whitespace-nowrap tracking-tighter cursor-default"
            ...
            <div className="mt-8 w-full h-px bg-[#23252B] hidden md:block"></div>
          </div>
          <div className="mt-8 md:mt-0 relative z-20">
            <a
              ...
              className="w-full bg-[#15171B] border border-[#23252B] rounded-full text-[#F5F5F5] px-6 py-4 flex items-center justify-between gap-4 text-xs font-semibold uppercase tracking-widest hover:border-[#D6FF4F] hover:text-[#D6FF4F] transition-colors group"
            >
              View Gallery
              <ArrowRight size={16} />
            </a>
